...
public class SudokuGame extends JFrame implements ActionListener{
	 ...
	 private JPanel addJtfToPanel(int subPanelNumber,JPanel subPanel, JTextField tf1,JTextField tf2, JTextField tf3, JTextField tf4, JTextField tf5, JTextField tf6,JTextField tf7, JTextField tf8, JTextField tf9){
	 ...
	 private JTextField[] makeJTFArr(JTextField tf1, JTextField tf2, JTextField tf3, JTextField tf4, JTextField tf5, JTextField tf6, JTextField tf7, JTextField tf8, JTextField tf9){
	     JTextField[] jtfArray = {tf1,tf2,tf3,tf4,tf5,tf6,tf7,tf8,tf9};
	     return jtfArray;
	 }
    @Override
	 public void actionPerformed(ActionEvent e){
	     // We want to populate the Textfield with the numbers here.
	     ...
	     // When we start a new game, we want to clear any pre-existing values on the board. Ensure board is empty
	     // Before populating
	     clearJTextFields();
	     resetJTfColors();
	     // Make generated JTF values non-editable
    ...
    // Makes all JTF have BG color white
    private void resetJTfColors(){
        // Color is red meaning other JTF in array have red color too. Clear all JTF colours.
        for(int row = 0; row<9; row++){
            for(int cols = 0; cols<9; cols++){
                jtfArray[row][cols].setBackground(Color.WHITE);
            }
        }
    }
    ...
    private void restrictGeneratedJTFAccess(SudokuBoard board){
          for(int row = 0; row<9; row++){
              for(int cols = 0; cols<9; cols++){
                  if(board.getFinalBoard()[row][cols]!=0){
                      JTextField temp = jtfArray[row][cols];
                      temp.setText(""+board.getFinalBoard()[row][cols]);
     ...
     */
     private JTextField constructJTextFields(JTextField jtf){
        ...
        // the background color that it is changed to is the same as it was before.
        ...
        KeyListener keyListener = new KeyListener() {
			...
			public void keyPressed(KeyEvent keyEvent) {
              ...
              // (i.e. "" if nothing only one changed occurred). The keyReleased method solves this.
			...
			private boolean isvalidCharacter(KeyEvent e) {
               ...
               // Character.getNumericValue returns positive integer values above -1 if it is a valid integer. If not, it sends
           ...
           private int[] findJtfIn2DArray(JTextField textField){
               int returningArray[] = {-1,-1};
               for(int row = 0; row<9; row++){
                   for(int col = 0; col<9; col++){
                       if(textField.equals(jtfArray[row][col])){
                           returningArray[0] = row;
                           returningArray[1] = col;
			...
			public void keyReleased(KeyEvent keyEvent) {
               ...
               // JTF and you can then do another iteration to see if Row/Col has that number. Not sure about Box yet.
               ...
               // Now we need to check rows/column/box for equal number.
               ...
               // Clear all red JTF. We will recolor if needed very soon
               //resetJTFColour();
               if(areDupesInBox||areDupesInCol||areDupesInRow){
                   // Duplicates exist. Set all JTF that is = valueInputted to background color red and ensure that you cannot
           ...
           private void setJTFColorRed(String value, boolean dupesinBox, boolean dupesInCol, boolean dupesInRow,
                                       int row, int col){
               // We know the row and column of the JTF that is focused.
               if(dupesInRow){
